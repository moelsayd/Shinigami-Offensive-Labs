...
PORT = int(sys.argv[1]) if len(sys.argv) > 1 else 12055
ROOM_DIR = os.path.dirname(os.path.abspath(__file__))
FLAG_PART3 = "SHINIGAMI{mobile_enterprise_compromised}"  # العلم الكامل للاستدلال
WORKER_SECRET = b"worker_secret_456"
...
def jwt_verify_worker(token):
    try:
        ...
        elif kid == "worker_key": secret = WORKER_SECRET
        else: return None
    ...
    while True:
        if task_queue:
            ...
            if b"admin_pipeline" in task["content"]:
                memory_state[task["id"]] = FLAG_PART3
            else:
