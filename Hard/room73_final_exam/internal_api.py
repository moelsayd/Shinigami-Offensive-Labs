...
PORT = int(sys.argv[1]) if len(sys.argv) > 1 else 11507
ROOM_DIR = os.path.dirname(os.path.abspath(__file__))
FLAG_PART1 = "SHINIGAMI{enterprise_"
WORKER_URL = "http://127.0.0.1:11642"
SECRET = b"eclipse_secret_123"   # نفس المفتاح الضعيف
...
class InternalHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/user/info":
            ...
            self._serve_json({"role": "admin"})  # يعيد admin للجميع
        elif path == "/api/internal/secret":
            if not payload or payload.get("role") != "admin":
                self.send_error(403); return
            self._serve_text(FLAG_PART1)
        elif path == "/queue":
            self._serve_json({"status":"processing","next":"/worker/status","worker":WORKER_URL})
        elif path == "/worker/status":
            # يتطلب أن يتم استدعاؤه من داخل الشبكة
